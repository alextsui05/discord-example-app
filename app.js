...
async function postPinyinMessage(text, url, token, appId) {
...
async function cangjieResponse(body, env) {
  const inputOptions = Object.fromEntries(
    body.data.options.map((option) => [option.name, option.value]),
  );
  const options = {
...
async function postCangjieResult(text, url, token, appId) {
  const response = await fetch(url);
...
export default {
  ...
  async queue(batch, env, ctx) {
    for (const message of batch.messages) {
      ...
      if (message.body.type === "cangjie") {
        console.log("Cangjie message:", message.body.data);
        await postCangjieResult(
          message.body.data.text,
          message.body.data.url,
          message.body.data.token,
          env.APP_ID,
